...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        # SY: Added, because could not find NER dataset with train + eval.
        logger.info("Splitting raw_dataset to training and eval")

        # The first 65% of `train` split will go to training dataset.
        training_dataset = load_dataset(data_args.dataset_name, 
                                        data_args.dataset_config_name, 
                                        split='train[:90%]')

        logger.info("Training dataset: {}".format(training_dataset))

        # The last 35% of `train` split will go to eval dataset.
        eval_dataset = load_dataset(data_args.dataset_name, 
                                        data_args.dataset_config_name, 
                                        split='train[-20%:-10%]')

        logger.info("Eval dataset: {}".format(eval_dataset))

        predict_dataset = load_dataset(data_args.dataset_name, 
                                            data_args.dataset_config_name, 
                                            split='train[-10%:]')
        
        logger.info("Predict dataset: {}".format(predict_dataset))

        # SY: Commented out - not needed any more
        '''raw_datasets = load_dataset(
            data_args.dataset_name,
            data_args.dataset_config_name,) 
            #cache_dir=model_args.cache_dir, # SY: Removed
            #use_auth_token=True if model_args.use_auth_token else None,) # SY: Removed'''
    else:
        ...
        if trainer.is_world_process_zero():
            with open(output_expected_predictions_file, "w") as writer:
                for instance in predict_dataset:
                    writer.write("words: {}, ner: {}\n".format(instance["words"], instance["ner"]))
