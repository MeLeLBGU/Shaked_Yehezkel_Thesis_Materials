...
# SY: Added import to TokenClassifierOutput, QuestionAnsweringModelOutput
...
# SY: Added, Copied from huggingface github repo
class BertForTokenClassification(BertPreTrainedModel):
    """
    Bert Model with a token classification head on top (a linear layer on top of the hidden-states output) e.g. for
    Named-Entity-Recognition (NER) tasks.
    """
    # SY: Added 'args=None' and the referencing to it. Copied from the BertForSequenceClassification in this page
    def __init__(self, config, args=None):
...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def __init__(self, config, args=None):
        ...
        self.bert = BertModel(config, args) # SY: Removed add_pooling_layer=False
        self.qa_outputs = nn.Linear(config.hidden_size, config.num_labels)

        # SY: BertForQuestionAnswering does not have post_init....
        #           so init weights instead.
        self.init_weights()
